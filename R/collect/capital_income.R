...
psid_abridged <- collect_tv(psid_abridged, "capital_business_income_nd_rc", function(x, y) {
  out <-
    ...
    else if (y <= 1980) recode(x, -9999 %..% 99998 ~ keep,   99999 ~ topcode)
    else                recode(x, -99999 %..% 999998 ~ keep, 999999 ~ topcode)  # 1981-92
  rc(out, is.na(x), NA)
})
...
psid_abridged <- collect_tv(psid_abridged, "capital_business_income_nd_rp", function(x, y) {
  out <-
    if (y == 1993)      recode(x, -999999 %..% 9999998 ~ keep, 9999999 ~ topcode)
...
psid_abridged <- collect_tv(psid_abridged, "capital_business_income_nd_sp", function(x, y) {
  out <-
    if (y <= 1992)      recode(x, -99999 %..% 999998 ~ keep,  999999 ~ topcode)   # 1985-92
...
psid_abridged <- collect_tv(psid_abridged, "capital_rent_div_intst_trst_income_nd_rp", function(x, y) {
  out <-
    if (y <= 1975)      recode(x, 0 %..% 8 ~ keep, 9 ~ NA)
    else if (y <= 1982) recode(x, -9999 %..% 99998 ~ keep,   99999 ~ topcode)
    else                recode(x, -99999 %..% 999998 ~ keep, 999999 ~ topcode)  # 1983
  rc(out, is.na(x), NA)
})
psid_abridged <- collect_tv(psid_abridged, "capital_rent_div_intst_trst_income_nd_sp", function(x, y) {
  out <-
    if (y <= 1975)      recode(x, 0 %..% 8 ~ keep, 9 ~ NA)
    else if (y <= 1983) recode(x, -9999 %..% 99998 ~ keep,   99999 ~ topcode)
    else                recode(x, -99999 %..% 999998 ~ keep, 999999 ~ topcode)  # 1984-92
  rc(out, is.na(x), NA)
})
...
psid_abridged <- collect_tv(psid_abridged, "capital_div_intst_trst_income_nd_rp", function(x, y) {
  rc(recode(x, 1 %..% 999998 ~ keep, 0 ~ keep, 999999 ~ topcode), is.na(x), NA)
})
...
psid_abridged <- collect_tv(psid_abridged, "capital_rental_income_nd_rp", function(x, y) {
  out <-
    if (y <= 2009) recode(x, -99999 %..% 999998 ~ keep, 999999 ~ topcode)  # 1984-93, 2005-09
    else           recode(x, -99997 %..% 999997 ~ keep)
  rc(out, is.na(x), NA)
})
psid_abridged <- collect_tv(psid_abridged, "capital_rental_income_nd_sp", function(x, y) {  # 2005+
  out <-
    if (y <= 2009) recode(x, -99999 %..% 999998 ~ keep, 999999 ~ topcode)
    else           recode(x, -99997 %..% 999997 ~ keep)
  rc(out, is.na(x), NA)
})
...
capital_dit_rp <- function(x, y) {
  out <-
    if (y <= 2009) recode(x, 1 %..% 999998 ~ keep, 0 ~ keep, 999999 ~ topcode)  # 1993, 2005-09
