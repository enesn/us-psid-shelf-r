...
degr <- function(x, y) {              # severity / degree (4-cat reversed)
  out <- rep(-1, length(x))
  out <- rc(out, inlist(x, 7), 0); out <- rc(out, inlist(x, 5), 1)
  out <- rc(out, inlist(x, 3), 2); out <- rc(out, inlist(x, 1), 3)
  rc(out, inlist(x, 8, 9, 0) | is.na(x), NA)
}
mon_fn <- function(hi, extra = NULL) function(x, y) {  # month/type codes -> identity
  out <- rep(-1, length(x)); out <- rc(out, inrange(x, 1, hi), x)
  if (!is.null(extra)) out <- rc(out, inrange(x, extra[1], extra[2]), x)
  out <- rc(out, inlist(x, 97), 80)
  rc(out, inlist(x, 98, 99, 0) | is.na(x), NA)
}
cat4 <- function(x, y) {
  out <- rep(-1, length(x))
  out <- rc(out, inlist(x, 4), 0); out <- rc(out, inlist(x, 1), 1)
