...
psid_abridged <- collect_tv(psid_abridged, "finc_tot_nd", function(x, y, df) {
  out <-
    if (y >= 1968 && y <= 1969)
      ...
      recode(x, 2 %..% 99998 ~ keep, 1 ~ keep, 99999 ~ topcode)
    else if (y == 1980)
      recode(x, 2 %..% 999998 ~ keep, 1 ~ keep, 999999 ~ topcode)
    else if (y >= 1981 && y <= 1993)
      recode(x, 2 %..% 9999998 ~ keep, 1 ~ keep, 9999999 ~ topcode)
      ...
      recode(x, -999998 %..% 9999998 ~ keep, -999999 ~ keep, 9999999 ~ topcode)
    else  # 2011+
      recode(x, -999996 %..% 9999996 ~ keep, -999997 ~ keep, 9999997 ~ topcode)
  rc(out, is.na(x), NA)
})
