...
psid_abridged <- collect_tv(psid_abridged, "farm_grossrevenue_nd_rc", function(x, y) {
  out <-
    ...
    else if (y == 1983) recode(x, 1 %..% 99997 ~ keep,   0 ~ keep, 99998 ~ 9999999,   99999 ~ NA)
    else if (y <= 1992) recode(x, 1 %..% 999997 ~ keep,  0 ~ keep, 999998 ~ 9999999,  999999 ~ NA)
    else if (y == 1993) recode(x, 1 %..% 9999997 ~ keep, 0 ~ keep, 9999998 ~ 9999999, 9999999 ~ NA)
    else                recode(x, 1 %..% 9999996 ~ keep, 0 ~ keep, 9999997 ~ 9999999,
...
psid_abridged <- collect_tv(psid_abridged, "business_netprofit_nd_rc", function(x, y) {
  out <- recode(x, -999998 %..% 9999996 ~ keep, 9999997 ~ 9999999,
...
psid_abridged <- collect_tv(psid_abridged, "business_income_nd_rc", function(x, y) {
  out <-
    if (y == 1993)      recode(x, -999998 %..% 9999998 ~ keep, -999999 ~ NA, 9999999 ~ 9999999)
...
psid_abridged <- collect_tv(psid_abridged, "farm_income_nd_rc", function(x, y) {
  out <-
    ...
    else if (y == 1999) recode(x, -999999 %..% 9999998 ~ keep, 9999999 ~ 9999999)
    else if (y == 2001) recode(x, -99999 %..% 9999998 ~ keep,  9999999 ~ 9999999)
    else if (y <= 2005) recode(x, -99999 %..% 999998 ~ keep,   999999 ~ 9999999)
    else if (y <= 2009) recode(x, -999999 %..% 9999998 ~ keep, 9999999 ~ 9999999)
    else                recode(x, -999997 %..% 9999997 ~ keep)
  rc(out, is.na(x), NA)
})
...
psid_abridged <- collect_tv(psid_abridged, "taxable_income_nd_rc", function(x, y) {
  out <-
    if (y <= 1978)      recode(x, -9999 %..% 99998 ~ keep,    99999 ~ 9999999)
    else if (y <= 1980) recode(x, -99999 %..% 999998 ~ keep,  999999 ~ 9999999)
