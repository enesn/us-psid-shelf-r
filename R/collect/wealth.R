...
# Each era keeps a valid range plus a few sentinel codes (refused / top-code)
# that pass through as themselves.
# =====================================================================

# 3-era net (can be negative)
w3 <- function(x, y) {
  out <-
    if (y <= 2005)      recode(x, -99999998 %..% 999999998 ~ keep, c(-99999999, 0, 999999999) ~ keep)
    else if (y <= 2009) recode(x, -99999998 %..% 999999996 ~ keep, c(0, 999999997) ~ keep)
    else                recode(x, -99999997 %..% 999999997 ~ keep, 0 ~ keep)
  rc(out, is.na(x), NA)
}
# 3-era positive-only
wp3 <- function(x, y) {
  out <-
    if (y <= 2005)      recode(x, 1 %..% 999999998 ~ keep, c(0, 999999999) ~ keep)
    else if (y <= 2009) recode(x, 1 %..% 999999996 ~ keep, c(0, 999999997) ~ keep)
    else                recode(x, 1 %..% 999999997 ~ keep, 0 ~ keep)
  rc(out, is.na(x), NA)
}
# 2-era net (home equity)
w2 <- function(x, y) {
  out <-
    if (y <= 2009) recode(x, -99999998 %..% 999999996 ~ keep, c(0, 999999997) ~ keep)
    else           recode(x, -99999997 %..% 999999997 ~ keep, 0 ~ keep)
  rc(out, is.na(x), NA)
}
# single-era net
w1  <- function(x, y) rc(recode(x, -99999997 %..% 999999997 ~ keep, 0 ~ keep), is.na(x), NA)
# single-era positive
wp1 <- function(x, y) rc(recode(x, 1 %..% 999999997 ~ keep, 0 ~ keep), is.na(x), NA)
